...
(defun node2var (n c k)
  ;simply apply the formula given to compute the variable node value
  (+ ( * (- n 1) k) c)
)
...
(defun at-least-one-color (n c k)
  ...
  (cond
    ;if color exceeds limit provided, stop.
    ((> c k) NIL)
...
;HELPER FUNCTION to check pairs of colors to see if either n doesn't have color a or n doesn't have color b.
(defun at-most-one-color-help (n a b k)
  (cond
    ;if the first color in the pair exceeds max, stop looking.
    ((>= a k) NIL)

    ;if the second color in the pair exceeds max, set it to prev + 1
    ((> b k) (at-most-one-color-help n (+ a 1) (+ a 2) k))

    ;n doesn't have color a or n doesn't have color b
    (T (append (list (list (- (node2var n a k)) (- (node2var n b k))))(at-most-one-color-help n a (+ b 1) k)))

  )
)

(defun at-most-one-color (n c k)
  ;use helper functions to check pairs of colors
  (at-most-one-color-help n c (+ c 1) k)
)
...
;HELPER FUNCTION: for a particular color
(defun generate-edge-clauses-help (a b c k)
  (cond

    ;if color exceeds max, stop.
    ((> c k) NIL)

    (T (append (list (list (- (node2var a c k)) (- (node2var b c k))))(generate-edge-clauses-help a b (+ c 1) k)))
    )
)


(defun generate-edge-clauses (e k)
  ;call helper function with first element and cadr of the edge.
  (generate-edge-clauses-help (car e) (cadr e) 1 k)
  )
